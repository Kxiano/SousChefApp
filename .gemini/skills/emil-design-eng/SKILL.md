...
Popovers should scale in from their trigger, not from center. The default `transform-origin: center` causes a disconnect between where the user clicked and where the element appears.
...
Modals are the exception — they stay centered because they are not triggered from a specific point.

### GPU-only properties

Only animate `transform` and `opacity`. These are composited by the GPU and never trigger layout.

Animating `width`, `height`, `top`, `left`, `margin`, `padding` triggers layout recalculation and causes jank.

### Accessibility

Always honor `prefers-reduced-motion`. Keep opacity and color transitions; remove spatial movement.
...
Gate hover animations behind pointer detection:
